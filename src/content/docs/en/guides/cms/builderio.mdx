...
## regex tests
### [example 2](https://github.com/withastro/docs/blob/main/contributor-guides/writing-and-style-guide.md#example-2)

* also: discrepancy between `props.social` and `props.socialLinks`?

```astro /{props.(title|socialLinks)}/ ins="<Button />"
...
**result**
It highlights something, but only `title` and `socialLinks` not the whole `{props.title}` as stated in our guide.
...
```astro del="My blog title"
--
const title = "My blog title"
const title = import.meta.env.BLOG_TITLE
--
...
**Result**
Seems to work! Although I kind of expected it to highlight the quotes as well

### ins="Astro.props"

```astro ins="Astro.props"
--
...
```astro /{frontmatter.(title|description)}/
--
...
**Result**
It works. I have a better understanding of the syntax now after I worked through the examples systematically like this. But only then :). 
